...
class UntilExpressionEvaluator(ctx: LeonContext, prog: Program) extends RecursiveEvaluator(ctx, prog) {
  override val name = "until expression evaluator"
  override val description = "Interpreter until a given expression for PureScala expressions"

  type RC = DefaultRecContext
  type GC = DefaultGlobalContext

  def initRC(mappings: Map[Identifier, Expr]) = DefaultRecContext(mappings)
  def initGC(until: Expr) = new DefaultGlobalContext(until, stepsLeft = -1)

  var recctx: RC

  def eval(ex: Expr, until: Expr, mappings: Map[Identifier, Expr]) = {
    try {
      EvaluationResults.Successful(e(ex)(initRC(mappings), initGC(until)))
    } catch {
  ...
  case class DefaultGlobalContext(until: Expr, stepsLeft: Int) extends GlobalContext(stepsLeft) {}

  def evalUntil(ex: Expr, until: Expr, mappings: Map[Identifier, Expr]) = {
    try {
      EvaluationResults.Successful(e(ex)(initRC(mappings), initGC(until)))
      recctx
    } catch {
  ...
  def e(expr: Expr)(implicit rctx: RC, gctx: GC) = {
    if (expr == gctx.until) {
      recctx = rctx
      expr
    } else {
      expr match {
        ...
        case FunctionInvocation(tfd, args) =>
          ...
          if (args.forall(isLiteral)) {
            ...
            FunctionInvocation(tfd, args.map(e))
          }
        ...
        case CaseClassInstanceOf(cct, expr) =>
          expr match {
            ...
            case _ => CaseClassInstanceOf(cct, e(expr))
          }
        ...
        case CaseClassSelector(ct1, expr, sel) =>
          expr match {
            ...
            case _ =>
              CaseClassSelector(ct1, e(expr), sel)
          }

        case Plus(l, r) =>
          (l, r) match {
            case (IntLiteral(i1), IntLiteral(i2)) => IntLiteral(i1 + i2)
            case (l, r) => Plus(e(l), e(r))
          }

        case Minus(l, r) =>
          (l, r) match {
            case (IntLiteral(i1), IntLiteral(i2)) => IntLiteral(i1 - i2)
            case (l, r) => Minus(e(l), e(r))
          }
        ...
        case Modulo(l, r) =>
          (l, r) match {
            ...
            case (IntLiteral(i1), IntLiteral(i2)) => IntLiteral(i1 % i2)
            case (l, r) => Modulo(e(l), e(r))
          }

        case LessThan(l, r) =>
          (l, r) match {
            case (IntLiteral(i1), IntLiteral(i2)) => BooleanLiteral(i1 < i2)
            case (l, r) => LessThan(e(l), e(r))
          }

        case GreaterThan(l, r) =>
          (l, r) match {
            case (IntLiteral(i1), IntLiteral(i2)) => BooleanLiteral(i1 > i2)
            case (l, r) => GreaterThan(e(l), e(r))
          }

        case LessEquals(l, r) =>
          (l, r) match {
            case (IntLiteral(i1), IntLiteral(i2)) => BooleanLiteral(i1 <= i2)
            case (l, r) => LessEquals(e(l), e(r))
          }

        case GreaterEquals(l, r) =>
          (l, r) match {
            case (IntLiteral(i1), IntLiteral(i2)) => BooleanLiteral(i1 >= i2)
            case (l, r) => GreaterEquals(e(l), e(r))
          }

        case SetUnion(s1, s2) =>
          (s1, s2) match {
            case (f @ FiniteSet(els1), FiniteSet(els2)) => FiniteSet((els1 ++ els2).distinct).setType(f.getType)
            case (l, r) => SetUnion(e(l), e(r))
          }
        ...
        case SetIntersection(s1, s2) =>
          (s1, s2) match {
            ...
            case (l, r) => SetIntersection(e(l), e(r))
          }
